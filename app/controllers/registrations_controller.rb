class RegistrationsController < Devise::RegistrationsController
  ...
  def generate_profile_suggestions
    ProfileSuggestionsGeneratorJob.perform_later(resource, true, 50) unless resource.id.nil?
  end

  def configure_permitted_parameters
    devise_parameter_sanitizer.permit(:sign_up, keys: %i[title first_name last_name email password age_bracket gender])
  end
