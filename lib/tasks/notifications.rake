namespace :notifications do
	task :daily_follows => :environment do
		items = DailyUserMailItem.select(:id, :user_id).where(action_type: "follow").where("DATE(created_at) = CURDATE()").group(:user_id)
		items.each do |item|
	...
	task :daily_categorisations => :environment do
		items = DailyUserMailItem.select(:id, :user_id).where(action_type: "categorisation").where("DATE(created_at) = CURDATE()").group(:user_id)
		items.each do |item|
